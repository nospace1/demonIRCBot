...
def createswarm(dictRef):
    ...
    if(noExistingSwarm(dictRef['bot'])):
        ...
        if(len(swarmlingNames) < 6):

            #first createimp called swarm
            #dictRef['data'] should equal 'botname createimp swarm'
            swarmRef = dictRef
            ...
            dictRef['bot'].talk(dictRef['where'], "Too large of swarmling name, needs to be 5 or less ")
    else:
